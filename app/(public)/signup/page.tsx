...
import { Users, Mic, ArrowRight, Check } from 'lucide-react'
...
function ExplorerForm({ onBack }: { onBack: () => void }) {
  ...
  const step1Valid = formData.firstName && formData.lastName && formData.email &&
    formData.phone && formData.age > 0 && formData.username &&
    formData.password.length >= 8 && formData.gender
  ...
  const nextStep = (current: number, valid: boolean | string | number) => {
    if (!valid) {
    ...
    <div className="min-h-screen flex pt-16">
      ...
      <div className="w-96 flex-shrink-0 bg-orange-50 border-r border-orange-100 sticky top-16 h-[calc(100vh-64px)] flex flex-col justify-between p-10 overflow-hidden">
        <div className="relative z-10">
          <button onClick={onBack} className="flex items-center gap-2 text-sm font-semibold text-gray-500 hover:text-orange-500 transition-colors mb-8">
            ← Back
          </button>
          <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-orange-100 border border-orange-200 rounded-full text-xs font-bold text-orange-600 uppercase tracking-wider mb-5">
            Explorer Sign Up
          </div>
          <h2 className="text-3xl font-extrabold text-gray-900 tracking-tight leading-tight mb-3">
            Join the<br /><span className="text-orange-500">crew.</span>
          </h2>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <input type="tel" placeholder="+234 800 000 0000" value={formData.phone} onChange={e => update('phone', e.target.value)} className={inputClass} />
                </div>
                <div>
...
function OrganiserForm({ onBack }: { onBack: () => void }) {
  ...
  const formValid = !!(formData.orgName && formData.contactName && formData.role &&
    formData.email && formData.phone && formData.password.length >= 8 &&
  ...
  const handleSubmit = async () => {
    if (!formValid) {
      setError('Please fill in all required fields, select at least one event type, and accept the terms.')
      return
    }
    setLoading(true)
    setError('')
    ...
    <div className="min-h-screen flex pt-16">
      ...
      <div className="w-96 flex-shrink-0 bg-blue-50 border-r border-blue-100 sticky top-16 h-[calc(100vh-64px)] flex flex-col justify-between p-10 overflow-hidden">
        <div className="relative z-10">
          ...
          <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-blue-100 border border-blue-200 rounded-full text-xs font-bold text-blue-600 uppercase tracking-wider mb-5">
            Organiser Sign Up
          </div>
          <h2 className="text-3xl font-extrabold text-gray-900 tracking-tight leading-tight mb-3">
            List your<br /><span className="text-blue-500">events.</span>
          </h2>
          ...
          {!submitted ? (
                  ...
                  {eventTypes.map(type => (
                    <button key={type}
                      ...
                      className={`px-4 py-2 rounded-full text-xs font-semibold border transition-all ${
                        selected.includes(type) ? 'bg-blue-50 border-blue-300 text-blue-600' : 'bg-gray-50 border-gray-200 text-gray-600 hover:border-gray-300'
                      }`}
                    >{type}</button>
                  ))}
                </div>
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-2xl font-extrabold text-gray-900 mb-3 tracking-tight">
                Application <span className="text-blue-500">submitted!</span>
              </h3>
              <p className="text-sm text-gray-500 mb-8 leading-relaxed max-w-sm mx-auto">
                Your application is under review. Our team will reach out within 24 to 48 hours to complete your verification.
              </p>
