...
function DatePickerDropdown({
  ...
  const buildHref = (dateStr: string) => {
    const params = new URLSearchParams()
    Object.entries(currentParams).forEach(([k, v]) => {
      if (v && k !== 'date') params.set(k, v)
    })
  ...
  return (
    <div className="relative flex-shrink-0" ref={ref}>
      ...
      {open && (
        <div className="absolute top-[calc(100%+8px)] left-0 bg-white border border-gray-200 rounded-2xl shadow-2xl z-[999] p-4 w-72">
          {/* Month navigation */}
          <div className="flex items-center justify-between mb-4">
            <button onClick={prevMonth} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 transition-colors text-gray-600 font-bold">‹</button>
            <span className="text-sm font-bold text-gray-900">{monthName}</span>
            <button onClick={nextMonth} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 transition-colors text-gray-600 font-bold">›</button>
          </div>
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    setOpen(false)
                    onSelect(buildHref(dateStr))
                  }}
                  className={`h-8 w-8 mx-auto flex items-center justify-center rounded-full text-xs font-semibold transition-all ${
                    isSelected
          ...
          {currentDate && (
            <button
              onClick={() => { setOpen(false); clearDate() }}
              className="w-full mt-3 py-2 text-xs font-bold text-orange-500 hover:underline"
            >
              Clear date
            </button>
          )}
    ...
    <div className="bg-white border-b border-gray-100">
      <div className="max-w-6xl mx-auto px-6 md:px-10 py-6">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <DatePickerDropdown
            currentDate={currentParams.date}
            currentParams={currentParams}
            onSelect={handleSelect}
          />
